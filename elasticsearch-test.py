...
from indexer import JSONSerializerPython2
...
if __name__ == '__main__':
    ...
    s = Search(using=es).index('book-index').doc_type('book').query("match", description='prince')
    #s = Search(using=es).index('book-index').doc_type('book')
    ...
    for res in response:
        ...
        pp(res.userreviews_userReviewDate)
        pp(res.userreviews_userURL)
        pp(res.type)
        print ''
